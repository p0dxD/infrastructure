...
P12KEY_NAME="website"
FULLCHAIN_NAME="fullchain"
CERTS_DIR=".certs"
LOCATION="/etc/letsencrypt/live/joserod.space/"
if [ "$1" ]; then
	certbot renew --cert-name joserod.space --dry-run
fi
#copy the certs
cp $LOCATION"$FULLCHAIN_NAME".pem . && chmod +r "$FULLCHAIN_NAME".pem
cp $LOCATION"$PRIVKEY_NAME".pem . && chmod +r "$PRIVKEY_NAME".pem
...
#move cert for jenkins
cp /home/p0dxd/.certs/website.jks /mnt/jenkins/var/jenkins_home
rm -f *.pem *.p12 *.jks
